...
pipe = pipeline("text-classification", model="iknow-lab/ko-flan-zero-v0-0731")

def inference(instruction, input, labels):
    instructions = [f"{input} [SEP] {instruction} [SEP] {label} [SEP]" for label in labels]
    scores = pipe(instructions)

    print(instruction, scores)
