...
class KoFlanEvalDataset(Dataset):
    ...
    def __getitem__(self, index) -> Dict:
        ...
        inputs = self.tokenizer(instruction, item['answer'], truncation=True, max_length=self.max_length)
        return inputs
...
def main(
    ...
    df_score = df_score.groupby(["task", "id", "answer_type"])["score"].apply(list).reset_index()
    df_score.score = df_score.score.map(max)
    df_score = df_score.pivot(index=["task", "id"], columns="answer_type", values="score")
    print(df_score)
