...
class ABSAGenerator(BaseGenerator):
    ...
    def generate(self, split: str):
        dataset = load_dataset(
            "iknow-lab/nikl_absa_2021_v1.1", split="train", use_auth_token=True
        ).shuffle(seed=42)
        ...
        for item in dataset:
            ...
            for opinion in item["opinions"]:
                ...
                # 무작위로 instance를 고른다
                if property == "일반":
                    instruction = random.choice(self.instructions_normal).format(
                        target=target
                    )
                else:
                    instruction = random.choice(self.instructions_property).format(
