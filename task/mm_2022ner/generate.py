...
class mm2022NERGenerator(BaseGenerator):
    ...
    def generate(self, split: str):
        dataset = load_dataset(
            "iknow-lab/mm_2022NER", split="train", use_auth_token=True
        ).shuffle(seed=42)
        dataset = dataset.train_test_split(test_size=0.1)[split]
        self.labelList = list(set([x["NER"] for x in dataset]))
        for item in dataset:
            # 무작위로 instance를 고른다
            instruction = random.choice(self.instructions)
            text = f"문장: \"{item['Sentence']}\", 개체: {item['Entity']}"
            pos = item["NER"]
